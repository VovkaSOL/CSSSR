...
class UnluckyVassal {
    public void printReportForKing(List<String> pollResults) {
        TreeMap<String, TreeSet<String>> отчётДляКороля = new TreeMap<>();
        pollResults.forEach(e -> {
                    Map.Entry<String, TreeSet<String>> хозяинИслуги = обработатьДанныеОтОдногоСлуги(e);
                    if (хозяинИслуги != null) отчётДляКороля.put(хозяинИслуги.getKey(), хозяинИслуги.getValue());
                }
        );

        System.out.println("Король");
        отчётДляКороля.forEach((хозяин, слуги) -> {
            System.out.println("    " + хозяин);
            if (слуги.size() != 0) {
                слуги.forEach(слуга ->
                        System.out.println("          " + слуга));
            }
        });

    }

    Map.Entry<String, TreeSet<String>> обработатьДанныеОтОдногоСлуги(String rawЛюди) {
        if (rawЛюди == null) return null; //кто к нам с null придёт тот от null и погибнет
        String[] людиОтдельноОтХозяина = rawЛюди.split(":");
        String хозяин = людиОтдельноОтХозяина[0];
        TreeSet<String> слуги = new TreeSet<>();
        if(людиОтдельноОтХозяина.length>1) {
            String люди[] = людиОтдельноОтХозяина[1].split(",");
            for (int i = 0; i < люди.length; i++) {
                слуги.add(люди[i].trim());
            }
        }
        return new AbstractMap.SimpleEntry<>(хозяин,слуги);
    }
}